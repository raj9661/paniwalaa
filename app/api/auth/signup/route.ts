...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, password, name, phone, role, deviceFingerprint: clientFingerprint } = body
    ...
    // Check if device already has an account (prevent multiple accounts from same device)
    ...
    if (existingDeviceUser) {
      return NextResponse.json(
        { error: 'An account already exists from this device. Each device can only have one active account. Please login to your existing account.' },
        { status: 400 }
      )
    }
